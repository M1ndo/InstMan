...
class InstMan():
    ...
    def user_file(self, userid):
        ...
        user_post = Path(f"~/.config/instman/userdata/{userid}/postdata.yml").expanduser()
            ...
            for media in media_list:
                ...
                with tqdm(total=media.itemcount, desc=f"Downloading {media_type} {media_title}", colour="#6a5acd", unit_scale=True, unit="MB", smoothing=0.5) as pbar:
                    for item in media.get_items():
                        path = f"media/{media.owner_username}/{media_type.lower()}s/" if media_type == "Story" else f"media/{media.owner_username}/{media_type.lower()}s/{media.title}/"
                        if self.api.download_storyitem(item=item, target=Path(path)):
                            pbar.update(1)
        return media_list

    # def get_posts(self, profile: Profile, comments:bool=False, download:bool=False) -> Tuple[List[Post], List[PostComment]]:
    #     """ Retrieve/Download User posts """
    #     posts = [post for post in profile.get_posts()]
    #     lcomments = []
    #     if download:
    #         logging.info(f"Downloading {profile.username} Posts [{len(posts)}].")
    #         with tqdm(total=len(posts), desc=f"Downloading {profile.username} Posts [{len(posts)}]", colour="#008b8b", unit_scale=True, unit="MB", smoothing=0.5) as pbar:
    #             count = 0
    #             for post in posts:
    #                 if comments:
    #                     lcomments.append({"postcode": post.shortcode, "postcomment": []})
    #                     logging.debug(f"Getting {post.caption} Comments")
    #                     for comment in post.get_comments():
    #                         lcomments[count]['postcomment'].append(comment)
    #                 count += 1
    #                 post_dir = post.date_local.strftime("%Y-%m-%d_%H%M%S")
    #                 path = f"media/{profile.username}/posts/{post_dir}/"
    #                 if self.api.download_post(post=post, target=Path(path)):
    #                     pbar.update(1)
    #                 else:
    #                     logging.debug(f"Post {post.pcaption} is already downloaded.")
    #                     pbar.update(1)
    ...
    def list_media(self, user, highlights:bool=False, stories:bool=False, posts:bool=False, comments:bool=False, download:bool=False):
        ...
        if posts:
            r_posts, r_comments = self.get_posts(profile, comments=True, download=download)
            self.handle_posts(r_posts, r_comments)
        # self.mark_changes(data=data,userid=str(userid[0]))
        return None

    def handle_posts(self, posts: List[Post], comments: List[Dict[str, Union[str, List[PostComment]]]]):
    ...
    def mark_changes(self, data: List[dict], userid: str) -> None:
        """ Mark Changes from stories/highlights/posts """
        _, _, post_file = self.user_file(userid)
        post_data, _ = self.read_data(post_file, userid)
        if not post_data:
            post_data = []
        post_data.extend(data)
        self.write_data_to_file(data=post_data, user_file=post_file)
        return None
